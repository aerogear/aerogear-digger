...
# - Script taken from https://github.com/openshift/jenkins/blob/master/2/contrib/jenkins/install-plugins.sh
# - The connection timeout is 5 seconds on that file and that is too short!
# - Connection is slow on OpenShift pods for some reason
#
#
...
function doDownload() {
    ...
    curl --connect-timeout 50 --retry 5 --retry-delay 0 --retry-max-time 60 -s -f -L "$url" -o "$jpi"
    return $?
}
